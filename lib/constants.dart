import 'package:flutter/material.dart';

Color btnColor = const Color(0xff00B0B0);
Color titleColor = const Color(0xffFF0000);
Color lightGrey = const Color(0xffDADADA);
Color bgColor = const Color(0xff051414);
Color whiteColor = const Color(0xffffffff);
Color drawerColor = const Color(0xff405454);

String dummyText =
    "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Diam maecenas mi non sed ut odio. Non, justo, sed facilisi et. Eget viverra urna, vestibulum egestas faucibus egestas. ";
double defaultPadding= 16.0;